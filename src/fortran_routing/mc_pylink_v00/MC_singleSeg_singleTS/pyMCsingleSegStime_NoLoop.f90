...
subroutine c_muskingcungenwm(dt, qup, quc, qdp, ql, dx, bw, tw, twcc,&
    ...
    real(c_double), intent(in) :: qup, quc, qdp, ql
    real(c_double), intent(in) :: dx, bw, tw, twcc, n, ncc, cs, s0
    real(c_double), intent(in) :: velp, depthp
    real(c_double), intent(out) :: qdc, velc, depthc
    real(c_double), intent(out) :: ck, cn, X
