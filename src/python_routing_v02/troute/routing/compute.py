...
def _prep_da_dataframes(
    ...
    Gages in `exclude_segments` will not be included in the
    returned indexes.
    ...
    """
    if exclude_segments:
        exclude_set = set(exclude_segments)
    else:
        exclude_set = set()
    ...
    if not usgs_df.empty and not lastobs_df.empty:
        ...
        lastobs_segs = list(lastobs_df.index.intersection(param_df_sub_idx).difference(exclude_set))
        lastobs_df_sub = lastobs_df.loc[lastobs_segs]
        usgs_segs = list(usgs_df.index.intersection(param_df_sub_idx).difference(exclude_set))
