...
def __compute_reach_kernel(qup, quc, nreach, input_buf, output_buf):
    ...
    for i in range(nreach):
        ...
        out = reach.compute_segment_kernel(
            dt, qup, quc, qdp, qlat, dx, bw, tw, twcc, n, ncc, cs, s0, velp, depthp,
        )
...
def compute_network(
    ...
    if qlat_values.shape[0] != parameter_idx.shape[0] or qlat_values.shape[1] != nsteps:
        raise ValueError(
            f"Qlat shape is incorrect: expected ({parameter_idx.shape[0], nsteps}), got ({qlat_values.shape[0], qlat_values.shape[1]})"
        )
    if (
        parameter_values.shape[0] != parameter_idx.shape[0]
        or parameter_values.shape[1] != parameter_cols.shape[0]
    ):
    ...
    print(
        f"cache_sizes {np.asarray(reach_group_cache_sizes)} reach_groups {np.asarray(reach_groups)}"
    )
    ...
    if 1 == 1:
        while timestep < nsteps:
            ...
            for group_i in range(len(reach_group_cache_sizes)):
                ...
                ireach_cache_end = (
                    ireach_cache
                    + reach_group_cache_sizes[group_i]
                    + reach_groups[group_i]
                )
                while ireach_cache < ireach_cache_end:
                    ...
                    for i in range(usreachlen):
                        quc += flowveldepth[
                            usreach_cache[iusreach_cache + i], ts_offset
                        ]
                        if timestep > 0:
                            qup += flowveldepth[
                                usreach_cache[iusreach_cache + i], ts_offset - 3
                            ]
                    ...
                    utils.fill_buffer_column(
                        srows, timestep, drows, 0, qlat_values, buf_view
                    )
                    for i in range(scols.shape[0]):
                        utils.fill_buffer_column(
                            srows, scols[i], drows, i + 1, parameter_values, buf_view
                        )
                        # fill buffer with qdp, depthp, velp
                    if timestep > 0:
                        utils.fill_buffer_column(
                            srows, ts_offset - 3, drows, 10, flowveldepth, buf_view
                        )
                        utils.fill_buffer_column(
                            srows, ts_offset - 2, drows, 11, flowveldepth, buf_view
                        )
                        utils.fill_buffer_column(
                            srows, ts_offset - 1, drows, 12, flowveldepth, buf_view
                        )
                    else:
                    ...
                    if timestep < 0:
                        print(
                            f"ts {timestep}, current reach_cache {reach_cache[ireach_cache]}, qup {qup}, quc {quc}, reachlen {reachlen}, buf_view {np.asarray(buf_view)}, out_view {np.asarray(out_view)}"
                        )
                    mc_reach.compute_reach_kernel(
                        qup, quc, reachlen, buf_view, out_view
                    )
                    if timestep == 1:
                        print(np.asarray(buf_view))
                        print(
                            f"ts {timestep}, reach {reaches[ireach]} segment indexes (reach_cache) {np.asarray(srows)}, qup {qup}, quc {quc}, reachlen {reachlen} ",
                            end="",
                        )
                        print(
                            f"upstream segments {[parameter_idx[r] for r in usreach_cache[iusreach_cache:iusreach_cache + usreachlen]]}"
                        )
                    ...
                    for i in range(3):
                        utils.fill_buffer_column(
                            drows, i, srows, ts_offset + i, out_view, flowveldepth
                        )
    ...
    return (
        np.asarray(parameter_idx, dtype=np.intp),
        np.asarray(flowveldepth, dtype="float32"),
    )
